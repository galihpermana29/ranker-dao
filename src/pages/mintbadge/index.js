...
import {connectWallet, getAccounts, addNetwork} from '../../services/provider';
...
const MintBadge = () => {
  ...
  useEffect(() => {
    if(chainId && Number(chainId) !== Number(process.env.REACT_APP_SUPPORTED_CHAIN_ID)) {
      addNetwork()
    }
  }, [chainId])
  ...
  const renderError = () => chainId && Number(chainId) !== Number(process.env.REACT_APP_SUPPORTED_CHAIN_ID) ? (
  ...
  const isButtonDisabled = () => {
    return (chainId && Number(chainId) !== Number(process.env.REACT_APP_SUPPORTED_CHAIN_ID)) || Number(balance) > 0 || !account || loading || txHash
  }
  ...
  const formatAccount = account => `${account.slice(0, 6)}...${account.slice(account.length - 6, account.length)}`
    ...
    <div className="container-fluid p-0">
      <div className="row p-0 m-0 hero-hero gx-5">
        ...
        <p className="akira-outlined-white">
              ...
              {account ? (<p className="bold-15 text-white my-4 d-none d-lg-block pad-right">
                  You are currently connected with <a
                    className="link"
                    href={`${process.env.REACT_APP_EXPLORER_URL}/address/${account}`}
                    target="_blank"
                    rel="noreferrer"
                  >{formatAccount(account)}</a>
                </p>
              ) : null}
              ...
              <div className="d-flex flex-row">
                {account ? (
                  <div className="px-0 d-none d-lg-block">
                    <button onClick={onDisconnect} className="connect-btn my-4 text-blue">DISCONNECT WALLET</button>
                  </div>
                ) : (
                  <div className="px-0 d-none d-lg-block">
                    <button onClick={onConnect} className="connect-btn my-4 text-blue">CONNECT WALLET</button>
                  </div>
                )}
                <div className="px-4 d-none d-lg-block">
                  <button onClick={isButtonDisabled() ? () => {} : onMint} className={`${getButtonClass()} my-4 bold-white`}>MINT YOUR BADGE</button>
                </div>
              </div>
